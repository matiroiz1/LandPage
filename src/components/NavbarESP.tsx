    ...
    <nav className="fixed top-0 left-0 w-full z-50 bg-[#0a0e14] border-b border-gray-800 shadow-md shadow-black/20">
      <div className="container mx-auto px-4 py-3 flex items-center justify-between rounded-full h-20 max-h-20 overflow-hidden">
        ...
        <div className="hidden md:flex items-center space-x-8">
          <Link href="#studies" className="hover:text-cyan-500 text-sm transition-colors duration-300">ESTUDIOS</Link>
          <Link href="#skills" className="hover:text-cyan-500 text-sm transition-colors duration-300">HABILIDADES</Link>
          <Link href="#portfolio" className="hover:text-cyan-500 text-sm transition-colors duration-300">PORTFOLIO</Link>
          <Link href="#contact" className="hover:text-cyan-500 text-sm transition-colors duration-300">CONTACTO</Link>
        </div>

        <div className="flex items-center">
          <Link href="/" className="bg-blue-500 text-white px-3 py-1 rounded-full text-xs mr-4 md:mr-0">
            ENG
          </Link>
        ...
        <div className={`absolute top-20 left-0 w-full bg-gray-900 z-50 transition-all duration-300 ease-in-out ${isOpen ? 'block' : 'hidden'} md:hidden`}>
          <div className="px-4 py-4 flex flex-col space-y-3">
            <Link href="#studies" className="hover:text-blue-300 text-sm" onClick={toggleMenu}>ESTUDIOS</Link>
            <Link href="#skills" className="hover:text-blue-300 text-sm" onClick={toggleMenu}>HABILIDADES</Link>
            <Link href="#portfolio" className="hover:text-blue-300 text-sm" onClick={toggleMenu}>PORTFOLIO</Link>
            <Link href="#contact" className="hover:text-blue-300 text-sm" onClick={toggleMenu}>CONTACTO</Link>
          </div>
        </div>
      </div>
    </nav>
